...
- 卡片头部 `- 阶段:` 字段记录**当前走到哪一步**,让 `kanban list`(working/done 列)一眼可见进度,对应 Multica 的 in_progress / in_review 概念。
- 取值:`实施中` → `待审核` → `审核中:PM` / `审核中:QA` / `审核中:CSA` / `审核中:Hacker` → `待验收` → `已验收`。
- **执行 Agent 在环节切换时用 `kanban stage <task-id> <阶段>` 推进**(不手改字段):开工/恢复=实施中,实现完成准备送审=待审核,每个角色审核开始=审核中:<角色>,审核通过待用户验收=待验收,验收完成=已验收。
- 旧卡无该字段时 `kanban list` 自动推断:实施完成(`工作结果: completed`)=待验收,进 done=已验收;auto 也会同步写入,人工任务可只靠推断不必手填。
...
- **依赖图从项目自身卡片与配置来, 不在 SKILL 里写死**: 调度参数(`--deps` 依赖图 / `--chain` / `--max` / `--interval`)是**项目级配置**, 每个项目不同, 且只应维护在一处。始终直接运行原生 `kanban auto`，不引入外部调度脚本：卡片建了任务组/「前置任务」字段时，无参 `kanban auto` 自动读取；都没有时，无参运行只做恢复并按 backlog/todo 顺序启动。
...
**分级通道**(卡 `- 审核:` 字段记录;`kanban new --review light|standard|full` 显式设定,默认标准):
- **默认链**: `标准` (standard) — PM → QA。逻辑、接口、数值和文案改动均沿用此链；CSA/Hacker 不因改动类型自动追加。
- **短链**(纯样式/纯演示: 颜色/间距/动效,不碰文案/逻辑/接口/数值): `轻量` (light) — 只跑 PM(或自审并记录结论),跳过 QA/CSA/Hacker。适合原型/演示/小改动/时间敏感。轻量≠免审: 仍要记录审核结论与未处理项。
- **完整链**: `完整` (full) — 仅当用户显式指定或确认升级时，PM → CSA/Hacker → QA 全跑；涉及资金时追加资金专项检查(金额/幂等/原子性/审计留痕/失败路径)。
...
- **阶段推进**: 每个环节切换时用 `kanban stage <task-id> <阶段>` 更新卡片头部阶段标签(实现完=待审核, 每个角色开始=审核中:<角色>, 全部通过=待验收), 让 `kanban list` 实时反映审核到哪一步; 用户验收确认后才填最终 `结果: completed` 并 `kanban move done`(此时 `已验收` 由 done 推断, 不必再手动 stage)。
...
1. 实现+必要审核完成后,**向用户请求验收**(集成前报告,非最终报告);用户确认前不集成、不清理、卡留 `working/`。
   - 使用原生 `kanban auto --gate work` 时，可在卡片写 `- 工作结果: completed` 表示实施完成：该卡仍留在 `working/` 等人工验收，但释放并发位；不要提前填写最终 `- 结果: completed`。
   - 仅明确标记 `- 验收: 自动` 的卡片，才允许由 auto 在实施完成后补最终结果并迁移 `done`。
