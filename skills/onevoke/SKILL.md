...
- **资金/安全/对外交付**: `完整` (full) — PM → CSA/Hacker → QA 全跑;资金相关(支付/钱包/结算/冲正)在全链之上加**资金专项检查**(金额/幂等/原子性/审计留痕/失败路径)。
...
- **值流向探针**: 对"纯核 + 宿主薄包装/模块常量/配置读取"分层,把宿主喂给纯核的值写死成常量,必须有断言翻红(同一形态已多次复发,全靠事后人肉抓)。
- **变异自证**: 关键判据要自变异——把被保护的行为拆掉/改坏,断言必须翻红;拆掉不红的判据等于没测。
- **红线检查**: 任务卡「红线」章节列明的禁止事项(不可改的模块/数据/环境/契约),验收时逐条核对未被触碰。
- **已知假红 vs 真回归**: 项目已知的随机假红(如统计类断言)先复跑定性,别当自己单的回归;但新出现的不明红必须查根因。
- **审查对抗性推演**: 审查含攻击者视角(怎么用这个改动"薅"系统)+ 跨模块串联(A 模块改动是否打破 B 模块假设);单轮视角必有盲区,高危问题可多轮独立审查。
