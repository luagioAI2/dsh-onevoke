...
## 验收条件(- [ ] 列表)    ## 威胁模型(非安全写 N/A)    ## 不在本轮范围
## 讨论与决策    ## 实施与验证    ## 完成总结(完成前留空)
```
...
- **L0 静态断言**(任务内): 关键元素存在、文案/结构正确——读代码即验。
- **L1 交互**(任务内): 点击/输入等行为符合预期——逻辑测试(mock DOM)或浏览器实测。
- **L2 接口**(任务内): 页面调用的后端接口行为正确——curl/实测。
- **L3 浏览器 E2E**(独立阶段,不在卡验收内): 任务完成、代码合并后,执行 `kanban l3 <task-id>` 单独验证:
  1. 启动应用并确认端口可访问;
...
- `kanban new --frontend` 会把 L0/L1/L2 写进卡验收条件;L3 用 `kanban l3` 独立执行,不在任务卡内。

## 审核(PM → QA,子代理只读执行)

**审核等级**(卡 `- 审核:` 字段,`kanban new --review light|standard|full` 设定;默认标准):
- `轻量` (light): 只跑 PM(或自审并记录结论),跳过 QA/CSA/Hacker。适合原型/演示/小改动/时间敏感。轻量≠免审: 仍要记录审核结论与未处理项。
- `标准` (standard): PM → QA,CSA/Hacker 按触发条件(默认)。
- `完整` (full): PM → CSA/Hacker → QA 全跑。适合安全敏感、对外交付、复杂改动。
...
2. 集成: rebase develop → 验证 → 审核通过结论沿用 → push `origin/develop` → 主树 ff-only merge → 清理 worktree/分支。
...
5. 发「看板任务完成报告」(8 字段,缺项写 无/N/A):
   ...
   - 收尾: <完整 SHA | N/A>; 集成结果; 主树同步/worktree/分支/临时审核文件/kanban check
