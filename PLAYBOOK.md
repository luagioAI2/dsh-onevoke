...
- 需求文件头部字段: 类型/模块/优先级/状态/需求来源/关联需求;必含章节: 需求描述/验收标准/
  (技术约束\|产出规范\|数值规范)/不在本轮范围/**红线**(禁止事项)/讨论与决策。旧式单文档(`## REQ-004`)仍兼容。
...
Agent 完成实现后会在任务窗口**请求验收**(含测试结果)。验收按**真值断言**(断言绿了缺陷还能不能存在?能存在就重写),禁凑绿;大任务 report 固定四段(改了哪些文件/测试怎么跑的/没解决的问题/自验证据)。你确认后回复(可粘贴):
